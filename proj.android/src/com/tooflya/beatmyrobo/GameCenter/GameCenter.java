package com.tooflya.beatmyrobo.GameCenter;
...
public abstract class GameCenter {
	...
	public static void postScore(final String idName, final int score) {
		...
		activity.runOnUiThread(new Runnable() {
			public void run() {
				...
				// if (rId > 0) {
				// String googleId = activity.getResources().getString(rId);
				getGameHelper().getGamesClient().submitScore(idName, score);
				// }
			}
		});
	}
	...
	public static void open(final String target) {
		...
		} else if (target.endsWith("store")) {
			try {
				Intent intent = new Intent(Intent.ACTION_VIEW);
				intent.setData(Uri
						.parse("market://details?id=com.tooflya.projectbirds"));
				activity.startActivity(intent);
			} catch (Exception e) {
				activity.startActivity(new Intent(
						Intent.ACTION_VIEW,
						Uri.parse("http://play.google.com/store/apps/details?id=com.tooflya.projectbirds")));
			}
	...
	public native static void onVideoPlayback();
}